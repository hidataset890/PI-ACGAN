...
function R = Polarization_fusion(I,m)
...
if (wexp_parm > 0)
    W = W.*well_brightness(I).^wexp_parm;
end
...
figure('Name','contrast measure');
for i = 1:N
    mono = rgb2gray(I(:,:,:,i));
    C(:,:,i) = abs(imfilter(mono,h,'replicate'));
    subplot(2,2,i); imshow(C(:,:,i));
end
...
N = size(I,4);
C = zeros(size(I,1),size(I,2),N);
figure('Name','saturation measure');
for i = 1:N
...
% well-brightness measure
function C = well_brightness(I)
sig = .2;
N = size(I,4);
C = zeros(size(I,1),size(I,2),N);
figure('Name','well-brightness measure');
for i = 1:N
