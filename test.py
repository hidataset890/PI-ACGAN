...
def get_parser():
    ...
    parser.add_argument('-i', '--image_path', type=str, default=None, help='file path of image you want to test')#./dataset/test/test_A
...
def test(G1, G2, test_dataset):
    ...
    for n, (img, gt_shadow, gt) in enumerate([test_dataset[i] for i in range(test_dataset.__len__())]):
        ...
        G1_weights = './checkpoints/PI-ACGAN_G1_10000.pth'
        # G1.load_state_dict(fix_model_state_dict(G1_weights))
        G1.load_state_dict(torch.load(G1_weights))  #torch.load   fix_model_state_dict
        G1.eval()
        G2_weights = './checkpoints/PI-ACGAN_G2_10000.pth'
        G2.load_state_dict(torch.load(G2_weights))
        with torch.no_grad():
...
def main(parser):
    ...
    if parser.load is not None:
        ...
        G1_weights = './checkpoints/PI-ACGAN_G1_10000.pth'
        #G1.load_state_dict(fix_model_state_dict(G1_weights))
        G1.load_state_dict(torch.load(G1_weights))
        G1.eval()
        G2_weights = torch.load('./checkpoints/PI-ACGAN_G2_10000.pth')
    ...
    if parser.image_path is not None:
        ...
        test_own_image(G1, G2, parser.image_path, parser.out_path, img_transform=ImageTransformOwn(mean=mean, std=std))
#, resized_size
