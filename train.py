...
from models.PI_ACGAN import Generator_r
from models.networks import PixelDiscriminator
...
def get_parser():
    parser = argparse.ArgumentParser(
        prog='PI-ACGAN: Polarization imaging shadow removal based on attention conditional generative adversarial networks',
        usage='python3 main.py',
        description='This module demonstrates shadow detection and removal using PI-ACGAN.',
        add_help=True)

    parser.add_argument('-e', '--epoch', type=int, default=10000, help='Number of epochs')#10000
    parser.add_argument('-b', '--batch_size', type=int, default=4, help='Batch size')#8
...
def evaluate(G1, G2, dataset, device, filename):
    ...
    with torch.no_grad():
        ...
        shadow_removal_image = shadow_removal_image.to(torch.device('cpu'))#'cpu'
    #save_image(gt_shadow, filename + '_detectS.jpg')
...
def main(parser):
    G1 = Generator_r(img_ch=3,output_ch=1)#img_ch=3,output_ch=1,input_channels=4, output_channels=3
    G2 = Generator_r(img_ch=4,output_ch=3)
    D1 = PixelDiscriminator(input_nc=4)#input_channels=7,input_nc=4
    D2 = PixelDiscriminator(input_nc=7)

    '''load'''
    if parser.load is not None:
        print('load checkpoint ' + parser.load)

        G1_weights = torch.load('./checkpoints/PI-ACGAN_G1_'+parser.load+'.pth')
        G1.load_state_dict(fix_model_state_dict(G1_weights))

        G2_weights = torch.load('./checkpoints/PI-ACGAN_G2_'+parser.load+'.pth')
        G2.load_state_dict(fix_model_state_dict(G2_weights))

        D1_weights = torch.load('./checkpoints/PI-ACGAN_D1_'+parser.load+'.pth')
        D1.load_state_dict(fix_model_state_dict(D1_weights))

        D2_weights = torch.load('./checkpoints/PI-ACGAN_D2_'+parser.load+'.pth')
    ...
    train_dataset = ImageDataset(img_list=train_img_list,
                                img_transform=ImageTransform( mean=mean, std=std),#size=size,, crop_size=crop_size
                                phase='train')
    ...
    G1_update, G2_update, D1_update, D2_update = train_model(G1, G2, D1, D2, dataloader=train_dataloader,
                                                            ...
                                                            parser=parser, save_model_name='PI-ACGAN')
